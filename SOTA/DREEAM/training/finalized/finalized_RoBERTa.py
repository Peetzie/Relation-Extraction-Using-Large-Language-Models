...
def run_training_pipeline(
    local_config, parts_to_run=["Part_1", "Part_2", "Part_3", "Part_4"]
):
    ...
    default_epochs_part_1 = 22
    default_lr_part_1 = 0.00002
    default_max_grad_norm_part_1 = 1.7
    # lambda_val = 0.15

    default_epochs_part_3 = 5.0
    default_lr_part_3 = 0.0002
    default_max_grad_norm_part_3 = 3.0
    # lambda_val = 0.14

    default_epochs_part_4 = 5
    default_lr_part_4 = 0.000048
    default_max_grad_norm_part_4 = 0.62
    lambda_val = 0.22
    ...
    def train(part):
        ...
        if part == "Part_1":
            ...
            teacher_save_path = f"/work3/s174159/LLM_Thesis/SOTA/DREEAM/wandb_out/{project_name.replace(' ', '_')}/teacher_output_seed_{seed_val}"
            run = wandb.init(
                project=project_name,
                name=f"1_{dataset_name.replace(' ', '_')}_Constraints_lambda{lambda_val}_{seed_val}",
                group="Part_1",
                reinit=True,
            )
            command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/1.sh '1_{dataset_name.replace(' ', '_')}_Constraints' {lambda_val} {seed_val} {teacher_save_path} {meta_dir} {data_dir} {num_class} {epochs} {lr} {max_grad_norm}"
        elif part == "Part_2":
            ...
            run = wandb.init(
                project=project_name,
                name=f"2_{dataset_name.replace(' ', '_')}_Infer_lambda{lambda_val}_{seed_val}",
                group="Part_2",
                reinit=True,
            )
            command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/2.sh '2_{dataset_name.replace(' ', '_')}_Infer' {teacher_ckpt_dir} {lambda_val} {seed_val} {meta_dir} {data_dir} {num_class}"
        elif part == "Part_3":
            # Step 3: Self-training on distantly-supervised data
            teacher_save_path = f"/work3/s174159/LLM_Thesis/SOTA/DREEAM/wandb_out/{project_name.replace(' ', '_')}/teacher_output_seed_{seed_val}"
            teacher_ckpt_dir = find_best_checkpoint(teacher_save_path)
            student_save_path = f"/work3/s174159/LLM_Thesis/SOTA/DREEAM/wandb_out/{project_name.replace(' ', '_')}/student_output_seed_{seed_val}"
            run = wandb.init(
                project=project_name,
                name=f"3_{dataset_name.replace(' ', '_')}_SelfTrain_lambda{lambda_val}_{seed_val}",
                group="Part_3",
                reinit=True,
            )
            command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/3RoBERTa.sh '3_{dataset_name.replace(' ', '_')}_SelfTrain' {teacher_ckpt_dir} {lambda_val} {seed_val} {student_save_path} {meta_dir} {data_dir} {num_class} {epochs} {lr} {max_grad_norm}"
        elif part == "Part_4":
            ...
            run = wandb.init(
                project=project_name,
                name=f"4_{dataset_name.replace(' ', '_')}_FineTune_lambda{lambda_val}_{seed_val}",
                group="Part_4",
                reinit=True,
            )
            command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/4RoBERTa.sh '4_{dataset_name.replace(' ', '_')}_FineTune' {student_ckpt_dir} {lambda_val} {seed_val} {student_save_path} {meta_dir} {data_dir} {num_class} {epochs} {lr} {max_grad_norm}"
...
if __name__ == "__main__":
    ...
    local_config_path = (
        "/work3/s174159/LLM_Thesis/SOTA/DREEAM/training/config/Roberta_Finalized.yml"
    )
    ...
    parser.add_argument(
        "-p", "--parts", nargs="+", required=False, default=[1, 2, 3, 4], type=int
    )
    ...
    for v in args.parts:
        parts_to_run.append(["Part_1", "Part_2", "Part_3", "Part_4"][v - 1])

    print(
        f"Running the following part{'' if len(parts_to_run)==1 else 's'}: {', '.join(parts_to_run)}"
    )
    ...
    run_training_pipeline(local_config, parts_to_run)

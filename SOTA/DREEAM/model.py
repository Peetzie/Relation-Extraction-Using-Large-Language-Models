...
class DocREModel(nn.Module):
    ...
    def forward_evi(self, doc_attn, sent_pos, batch_rel, offset):
        ...
        for i in range(len(sent_pos)):  # for each batch
            ...
            curr_sent_pos = [
                torch.arange(s[0], s[1]).to(curr_attn.device) + offset
                for s in sent_pos[i]
            ]  # + offset

            curr_attn_per_sent = [
                curr_attn.index_select(-1, sent) for sent in curr_sent_pos
            ]
            curr_attn_per_sent += [torch.zeros_like(curr_attn_per_sent[0])] * (
                max_sent_num - len(curr_attn_per_sent)
            )
            sum_attn = torch.stack(
                [attn.sum(dim=-1) for attn in curr_attn_per_sent], dim=-1
            )  # sum across those attentions
    ...
    def forward(
        ...
        # print("Sent label size", sent_labels.size())
        # print("labels", labels)
        if sent_labels != None:  # human-annotated evidence available

            s_attn = self.forward_evi(doc_attn, sent_pos, batch_rel, offset)
            output["evi_pred"] = F.pad(
                s_attn > self.evi_thresh, (0, self.max_sent_num - s_attn.shape[-1])
            )
        ...
        if tag == "infer":  # teacher model inference
            ...
            if sent_labels != None:  # supervised training with human evidence
                ...
                # print("Sent label size2", sent_labels.size())
                norm_s_labels = sent_labels / (
                    sent_labels.sum(dim=-1, keepdim=True) + 1e-30
                )

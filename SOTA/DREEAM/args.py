...
def add_args(parser):
    ...
    parser.add_argument(
        "--save_attn",
        action="store_true",
        help="Whether store the evidence distribution or not",
    )
    ...
    parser.add_argument(
        "--evi_thresh", default=0.2, type=float, help="Evidence Threshold. "
    )
    parser.add_argument(
        "--evi_lambda",
        default=0.1,
    ...
    parser.add_argument("--predictions_output", default=None, type=str)
...
globals.rel2id = json.load(open(f"{args.meta_dir}/df_rels.json", "r"))
globals.id2rel = {value: key for key, value in globals.rel2id.items()}
globals.ent2id = json.load(open(f"{args.meta_dir}/df_entities.json", "r"))

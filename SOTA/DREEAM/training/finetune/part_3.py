...
def train_part_3(local_config, teacher_save_path):
    ...
    sweep_config = {
        ...
        "metric": {"name": "rel_loss", "goal": "minimize"},  # Optimize for dev_f1
        "early_terminate": {"type": "hyperband", "min_iter": 2},
        "parameters": {
            "epochs": {"min": 2, "max": 5},  # Range of epochs
            "lr": {"min": 5e-6, "max": 5e-4},  # Learning rate range
    ...
    def train():
        ...
        command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/3.sh '3_{dataset_name.replace(' ', '_')}' {teacher_ckpt_dir} {lambda_val} {config.seed} {student_save_path} {meta_dir} {data_dir} {num_class} {config.epochs} {config.lr} {config.max_grad_norm}"

...
def create_sweep_config(seed_val, lambda_val, constraint_type):
    sweep_config = {
        ...
        "parameters": {
            ...
            "lr": {"values": [3e-5]},  # Replace with appropriate value
            "max_grad_norm": {"values": [5.0]},  # Replace with appropriate value
        },
    }
    return sweep_config
        ...
        def train(config=None):
            with wandb.init(config=config) as run:
                ...
                for dataset in local_config["datasets"]:
                    ...
                    command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/3.sh '3_{dataset_name.replace(' ', '_')}_SelfTrain' {teacher_ckpt_dir} {lambda_val} {config.seed} {student_save_path} {meta_dir} {data_dir} {num_class} {config.epochs} {config.lr} {config.max_grad_norm}"

...
def official_evaluate(
    ...
    if out_prediction_file is not None:
        ...
        inc = 0
    truth_iterator = (
        tqdm(truth, desc="Processing truth data")
        if out_prediction_file is not None
        else truth
    )
    for x in truth_iterator:
        title = x["title"]
        # titleset.add(title)
    ...
    submission_answer_filter = {
        (x["title"], x["r"], x["h_idx"], x["t_idx"]) for x in submission_answer
    }
    correct_re = 0
    correct_evidence = 0
    pred_evi = 0
    ...
    # Variables for macro and micro F1 calculation
    class_precisions = {}
    class_recalls = {}
    class_f1_scores = {}
    class_counts = {}

    class_precisions_out = {}
    class_recalls_out = {}
    sub_answer_iterator = (
        tqdm(submission_answer, desc="Processing truth data")
        if out_prediction_file is not None
        else submission_answer
    )
    for x in sub_answer_iterator:
        title = x["title"]
        h_idx = x["h_idx"]
        t_idx = x["t_idx"]
        r = x["r"]
        ...
        pred_evi += len(evi)
        if out_prediction_file is not None:
            # Initialize class precision and recall counters
            if r not in class_precisions:
                class_precisions[r] = {"TP": 0, "FP": 0}
                class_recalls[r] = {"TP": 0, "FN": 0}
                class_counts[r] = 0
        ...
        if (title, r, h_idx, t_idx) in std:
            if out_prediction_file is not None:
                class_precisions[r]["TP"] += 1  # True positive for precision
                class_recalls[r]["TP"] += 1  # True positive for recall
            correct_re += 1
            ...
            if out_prediction_file is not None:
                class_precisions[r]["FP"] += 1  # False positive for precision

        # elif (
        #     out_prediction_file is not None
        #     and (title, r, h_idx, t_idx) not in submission_answer_filter
        # ):
        #     class_recalls[r]["FN"] += 1  # False negative for recall
        # else:
        #     if out_prediction_file is not None:
        #         class_precisions[r]["FP"] += 1  # False positive for precision
    ...
    if out_prediction_file is not None:
        for title, r, h_idx, t_idx in std:
            if (title, r, h_idx, t_idx) not in submission_answer_filter:
                if r not in class_recalls:
                    class_recalls[r] = {"TP": 0, "FN": 0}
                class_recalls[r]["FN"] += 1

        # Now, calculate false negatives for recall
    ...
    if out_prediction_file is not None:
        for r in class_precisions:
            tp = class_precisions[r]["TP"]
            fp = class_precisions[r]["FP"]
            fn = class_recalls[r]["FN"]
            ...
            class_precisions_out[r] = precision
            class_recalls_out[r] = recall
            class_f1_scores[r] = f1
        # print("class_precisions_out", class_precisions_out)
        # print("class_recalls_out", class_recalls_out)
        # print("class_f1_scores", class_f1_scores)
    ...
    return (
        ...
        [class_precisions_out, class_recalls_out, class_f1_scores, 0],
    )

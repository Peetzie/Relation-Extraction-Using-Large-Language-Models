...
def train_part_4(local_config, student_save_path):
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "lr": {"min": 1e-7, "max": 5e-5},  # Learning rate range
            "max_grad_norm": {"min": 0.5, "max": 3.0},  # Gradient norm range
    ...
    def train():
        ...
        command = f"sh /work3/s174159/LLM_Thesis/SOTA/DREEAM/training/4.sh '4_{dataset_name.replace(' ', '_')}' {student_ckpt_dir} {config.lambda_} {config.seed} {student_save_path} {meta_dir} {data_dir} {num_class} {config.epochs} {config.lr} {config.max_grad_norm}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "student_save_path", metavar="S", type=str, help="The student save folder path"
    )
    ...
    train_part_4(local_config, args.student_save_path)

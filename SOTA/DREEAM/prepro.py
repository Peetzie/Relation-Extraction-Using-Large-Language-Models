...
# docred_rel2id = json.load(open("/work3/s174159/dreeam/meta/rel2id.json", "r"))
# docred_ent2id = {"NA": 0, "ORG": 1, "LOC": 2, "NUM": 3, "TIME": 4, "MISC": 5, "PER": 6}

# docred_rel2id = json.load(open('/work3/s174159/LLM_Thesis/processed_data/DocRED/meta/df_rels.json', 'r'))
# docred_ent2_id = json.load(open('/work3/s174159/LLM_Thesis/processed_data/DocRED/meta/df_entities.json', 'r'))
...
def get_pseudo_features(
    ...
    for pred_rel in pred_rels:
        ...
        if pred_rel["t_idx"] >= len(entities) or pred_rel["h_idx"] >= len(entities):
            print("********************")
            print(f"{entities}")
            print(f"{pred_rel}")
            print("********************")
        ...
        if (
            curr_sents in sent_grps
        ):  # skip if such sentence group has already been created
            continue
        ...
        new_input_ids_each = [
            raw_feature["input_ids"][s[0] + offset : s[1] + offset]
            for s in old_sent_pos
        ]
        ...
        for i, entity in enumerate(entities):
            ...
            for mention in entity:
                if mention["sent_id"] in curr_sents:
                    ...
                    pos = [
                        sent_map[mention["sent_id"]][pos]
                        - sent_map[mention["sent_id"]][0]
                        + prev_len
        ...
        for h in range(len(curr_entities)):
            for t in range(len(curr_entities)):
                if h != t:
                    ...
                    curr_label = raw_feature["labels"][
                        raw_feature["hts"].index([old_h, old_t])
                    ]
...
def read_docred(
    ...
    for doc_id in tqdm(range(len(data)), desc="Loading examples"):
        ...
        for h in range(len(entities)):
            for t in range(len(entities)):
                ...
                if h != t and [h, t] not in hts:  # and [t, h] not in hts:
                    relation = [1] + [0] * (len(globals.rel2id) - 1)
                    sent_evi = [0] * len(sent_pos)
                    relations.append(relation)
        ...
        #     len(entities) - 1
        # ), f"Relations:{relations},\n {entities}, \n Relation Len: {len(relations)} \n entityLen: {len(entities) * (len(entities) - 1)}"
        ...
        if (
            single_results != None
        ):  # get pseudo documents from predictions of the single run
    ...
    #     sent_labels = feature["sent_labels"]
    #     for i, sent_label in enumerate(sent_labels):
    #         if len(sent_label) == 0:
    #             print(
    #                 f"Empty sent_label found in document {feature['title']} at index {i}: {sent_label}"
    #             )

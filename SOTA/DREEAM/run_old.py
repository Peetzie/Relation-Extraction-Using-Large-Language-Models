...
from utils import set_seed, collate_fn, create_directory, collate_fn_fusion
...
def load_input(batch, device, tag="dev"):
    ...
    input = {
        ...
        "sent_labels": (
            batch[6].to(device)
            if (not batch[6] is None) and (batch[7] is None)
            else None
        ),
...
def train(args, model, train_features, dev_features):
        ...
        for epoch in tqdm(train_iterator, desc="Train epoch"):
            for step, batch in enumerate(train_dataloader):
                ...
                if inputs["sent_labels"] != None:
                    ...
                    # wandb.log({"evi_loss": outputs["loss"]["evi_loss"]})

                if inputs["teacher_attns"] != None:
                    loss.append(outputs["loss"]["attn_loss"] * args.attn_lambda)
                    # wandb.log({"attn_loss": outputs["loss"]["attn_loss"]})
                ...
                if (step + 1) == len(train_dataloader) or (
                    ...
                    (
                        ...
                        official_results,
                        results,
                    ) = evaluate(args, model, dev_features, tag="dev")
                    ...
                    if epoch == train_iterator[-1]:  # last epoch
                        ...
                        results_file = os.path.join(
                            args.save_path, f"topk_{args.pred_file}"
                        )
    ...
    optimizer = AdamW(
        optimizer_grouped_parameters, lr=args.lr_transformer, eps=args.adam_epsilon
    )
    num_steps = 0
    set_seed(args)
    model.zero_grad()
...
def evaluate(args, model, features, tag="dev", collate_fn=collate_fn):
    ...
    labels = np.concatenate(labels, axis=0)  ## Used for additional metrics.

    # print("**********************************")
    # print(globals.id2rel[argmax_preds[0]])
    # print(globals.id2rel[argmax_labels[0]])
    ...
    official_results, results = to_official(
        preds, features, evi_preds=evi_preds, scores=scores, topks=topks
    )
    ...
    fpr, tpr, _ = roc_curve(labels_flat, preds_flat, pos_label=1)
    auc = roc_auc_score(labels_flat, preds_flat)

    tn, fp, fn, tp = confusion_matrix(labels_flat, preds_flat).ravel()
    total = tp + fp + fn + tn

    # Calculate percentages for TP, FP, FN, TN
    tp_percentage = 100 * tp / total if total else 0
    fp_percentage = 100 * fp / total if total else 0
    fn_percentage = 100 * fn / total if total else 0
    tn_percentage = 100 * tn / total if total else 0

    if len(official_results) > 0:
        if tag == "dev":
            best_re, best_evi, best_re_ign, best_re_train, _ = official_evaluate(
                official_results, args.data_dir, args.train_file, args.dev_file
            )
        elif args.do_test:
            best_re, best_evi, best_re_ign, best_re_train, _ = official_evaluate(
                official_results,
        ...
        macro_f1 = best_re[-1]  # Extract macro F1 from best_re tuple
        # best_re = best_re[:-1]  # Remove macro F1 from best_re tuple
    else:
        best_re = best_evi = best_re_ign = [-1, -1, -1]
        macro_f1 = -1  # Default value if no results
    ...
    scores = {
        "dev_F1": best_re[2] * 100,
        "dev_evi_F1": best_evi[2] * 100,
        "dev_F1_ign": best_re_ign[2] * 100,
        "accuracy": accuracy,
        "MSE": mse,
        "RMSE": rmse,
        "AUC": auc,
    ...
    if args.save_attn:
        ...
        attns_path = os.path.join(
            args.load_path, f"{os.path.splitext(args.test_file)[0]}.attns"
        )
        print(f"saving attentions into {attns_path} ...")
        with open(attns_path, "wb") as f:
            pickle.dump(attns, f)

    return scores, output, official_results, results  # Return Macro F1 score
...
def dump_to_file(
    ...
    headers = ["precision", "recall", "F1", "Makro F1"]
...
def main():
    ...
    wandb.init(project=str(args.wandb_project), name=args.display_name)
    ...
    if args.do_train:  # Training
        ...
        if args.eval_mode != "fushion":
            ...
            results = json.load(
                open(os.path.join(args.load_path, f"topk_{args.pred_file}"))
            )
            ...
            merged_offi, thresh = merge_results(
                results, pseudo_results, test_features, thresh
            )
            merged_re, merged_evi, merged_re_ign, _, class_scores = official_evaluate(
                merged_offi, args.data_dir, args.train_file, args.test_file
            )
            ...
            merged_output.update(
                {
                    r: [class_scores[0][r], class_scores[1][r], class_scores[2][r]]
                    for r in class_scores[0].keys()
                }
            )

            wandb.log(
                {
                    "dev_F1": merged_re[2] * 100,
                    "dev_evi_F1": merged_evi[2] * 100,
                    "dev_F1_ign": merged_re_ign[2] * 100,
                    "macro_F1": merged_re[3] * 100,  # Log Macro F1 score
                }
            )
            ...
            dump_to_file(
                merged_offi, offi_path, merged_output, score_path, thresh=thresh
            )

...
class BasePLModule(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.hparams.relations:
            f = open(self.hparams.relations)
            self.relations = json.load(f)
        if self.hparams.entities:
            f = open(self.hparams.entities)
    ...
    def generate_triples(
        ...
        gen_kwargs = {
            "max_length": (
                self.hparams.val_max_target_length
        ...
        elif self.hparams.dataset_name.split("/")[-1] == "custom.py":
            reverse_rels = {v: k for k, v in self.entities.items()}
            return [
                extract_triplets_typed(rel, reverse_rels) for rel in decoded_preds
            ], [extract_triplets_typed(rel, reverse_rels) for rel in decoded_labels]
    ...
    def validation_epoch_end(self, output: dict) -> Any:
        if self.hparams.relations_file:
            ...
            scores, precision, recall, f1, wandb_scores = re_score(
            ...
            wandb.log(wandb_scores)
        elif not "tacred" in self.hparams.dataset_name.split("/")[-1]:
            if self.hparams.dataset_name.split("/")[-1] == "CoNLL04_typed.py":
                # print("Output predictions", [(pred["labels"]) for pred in output])
                print(
                    "Length of list above:",
                    len([item for pred in output for item in pred["labels"]]),
                )
                scores, precision, recall, f1, wandb_scores = re_score(
                    [item for pred in output for item in pred["predictions"]],
                    [item for pred in output for item in pred["labels"]],
            ...
            prec_micro, recall_micro, f1_micro = score(
                labels_list, preds_list, verbose=True
            )
    ...
    def test_epoch_end(self, output: dict) -> Any:
        _inc = 0
        _lines = []
        for batched_predictions_labels in output:
            sent_predictions = batched_predictions_labels["predictions"]
            sent_labels = batched_predictions_labels["labels"]
            for predictions, labels in zip(sent_predictions, sent_labels):
        ...
        with open(os.path.join(self.hparams.output_dir, "predictions.json"), "w") as f:
        ...
        if not self.hparams.finetune and self.hparams.relations_file:
            ...
            print("Testing.....")
            print(self.hparams.output_dir)
            if not os.path.isdir(self.hparams.output_dir):
                os.mkdir(self.hparams.output_dir)
            with open(os.path.join(self.hparams.output_dir, "data.csv"), "w") as file:
                file.write("Class,tp,fp,fn,p,r,f1\n")
                for rel_type in scores.keys():
                    file.write(
                        f'"{rel_type}",{scores[rel_type]["tp"]},{scores[rel_type]["fp"]},{scores[rel_type]["fn"]},{scores[rel_type]["p"]},{scores[rel_type]["r"]},{scores[rel_type]["f1"]}\n'
                    )
                file.write("\n")
            self.log("test_prec_micro", precision)
            self.log("test_recall_micro", recall)
            self.log("test_F1_micro", f1)
            # wandb.log(wandb_scores) # complains that wandb.init hasn't been called before wand.log()

...
def parse_arguments():
    ...
    parser.add_argument(
        "gold_file", help="The gold relation file; one relation per line"
    )
    parser.add_argument(
        "pred_file",
...
def re_score(pred_relations, gt_relations, relation_types, mode="boundaries"):
    ...
    true_vals = [
        1 if any(rel["type"] != NO_RELATION for rel in sent) else 0
        for sent in gt_relations
    ]
    pred_vals = [
        1 if any(rel["type"] != NO_RELATION for rel in sent) else 0
        for sent in pred_relations
    ]
    mse = mean_squared_error(true_vals, pred_vals)
    rmse = np.sqrt(mse)

    # Accuracy: sum of correct predictions (tp + tn) over total predictions
    accuracy = (tp + len(true_vals) - (tp + fp + fn)) / len(true_vals)
    ...
    wandb_scores = {
        ...
        "f1_micro": f1,
        "accuracy": accuracy,
        "macro_f1": scores["ALL"]["Macro_f1"],
        "RMSE": rmse,
        "MSE": mse,
        "True_Positive (%)": tp * 100,
        "False_Positive (%)": fp * 100,
    ...
    return scores, precision, recall, f1, wandb_scores
...
if __name__ == "__main__":
    ...
    if len(prediction) != len(key):
        print(
            "Gold and prediction file must have same number of elements: {} in gold vs {} in prediction".format(
